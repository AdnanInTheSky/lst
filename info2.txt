// api/initiate.js
// POST /api/initiate
// 1. Validates customer + cart
// 2. Calculates total server-side
// 3. Saves order to MongoDB
// 4. Calls PayStation
// 5. Fires Meta CAPI InitiateCheckout (non-blocking)
// Returns { payment_url, invoice_number, event_id }
...
module.exports = async function handler(req, res) {
  ...
  const {
    ...
    division, jela, upojela, thana, address_detail,
    fbp, fbc, event_id,            // from browser pixel for deduplication
  } = req.body || {};
  ...
  const div     = sanitise(division, 50);
  const jl      = sanitise(jela, 50);
  const upj     = sanitise(upojela, 50);
  ...
  if (!div || !jl || !upj || !thn) return res.status(400).json({ error: "Division, Jela, Upojela and Thana are required" });
  ...
  // ── Generate invoice & build full address ────────────────────────────────────
  const invoice_number = generateInvoice();
  const full_address   = [detail, thn, upj, jl, div].filter(Boolean).join(", ");
  ...
  const orderDoc = {
    ...
    status:         "initiated",
    verified:       false,
    customer: { name, phone, email, division: div, jela: jl, upojela: upj, thana: thn, address_detail: detail, full_address },
    items:          lineItems,
    checkout_items,
    callback_url,
    payment_url:    null,
  ...
  if (ordersCol) {
    try {
      ...
      if (err.code === 11000) {
        console.error("Duplicate invoice (should never happen with ObjectId):", invoice_number);
      } else {
        console.error("MongoDB insertOne error:", err.message);
      }
      // Non-fatal — proceed to PayStation
    }
  }
